...
JsonFormater.prototype = {
    ...
    bindEvent: function () {
        var that = this;
        $(this.options.dom).off('click','.fontToggle');
        $(this.options.dom).on('click', '.fontToggle', function () {
    ...
    makeContentVisible : function(element, visible){
        var font = element.prev().find('font');
        if(visible){
            element.show();
            font.text(' - ')
            font.data('status', 1);
    ...
    ProcessObject: function (obj, indent, addComma, isArray, isPropertyContent) {
        ...
        if (this.isArray(obj)) {
            if (obj.length == 0) {
                ...
                clpsHtml = this.options.isCollapsible ? "<span><font class='fontToggle' data-status='1'> - </font></span><span class='jf-collapsible'>" : "";
